...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, index) + str.slice(index).replace(value, '');
}
...
function sumOfCodes(str) {
  let result = 0;
  if (str) {
    result = str.split('').reduce((sum, char) => sum + char.charCodeAt(), 0);
  }
  return result;
}
...
function isPalindrome(str) {
  ...
  const strNew = str.toLowerCase().replace(regex, '');

  for (let i = 0; i < strNew.length / 2; i += 1) {
